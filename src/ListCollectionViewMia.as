package
{
	 ...
	 *  @productversion Flex 3
	 */
	public class ListCollectionViewMia extends Proxy
		implements ICollectionView, IList, IMXMLObject
	{
		 ...
		 */
		public var autoUpdateCounter:int;
		 ...
		 *  @productversion Flex 3
		 */
		public function ListCollectionViewMia(list:IList = null)
		{
			super();
		...
		public function setItemAt2(item:Object, index:int, source:Array):Object
		{
			if (index < 0 || !list || index >= length)
			{
				var message:String = resourceManager.getString(
			...
			return ArrayListMia(list).setItemAt2(item, index);
		}
		...
		private function addItemsToView(items:Array, sourceLocation:int,
										dispatch:Boolean = true, sortEnable:Boolean = true):int
		{
		...
		private function listChangeHandler(event:CollectionEvent):void
		{
			...
			else
			{
				switch (event.kind)
				{
					...
					case "replace_source":
						replaceItemsInView2(event.items, event.location);
						break;
		 ...
		 *  @productversion Flex 3
		 */
		private function removeItemsFromView(items:Array, sourceLocation:int, dispatch:Boolean = true):int
		{
		...
		private function replaceItemsInView2(items:Array,
											location:int,
											dispatch:Boolean = true):void
		{
			...
			if (localIndex)
			{
				...
				;
				addItemsToView(newItems, removeItemsFromView(oldItems, location, dispatch), dispatch, false);
			}
			else
			{
				var event:CollectionEvent =
...
class ListCollectionViewCursor extends EventDispatcher implements IViewCursor
{
	 ...
	 *  @productversion Flex 3
	 */
	public function ListCollectionViewCursor(view:ListCollectionViewMia)
	{
		super();
	 ...
	 *  @private
	 */
	private var _view:ListCollectionViewMia;
...
class ListCollectionViewBookmark extends CursorBookmark
{
	...
	mx_internal var view:ListCollectionViewMia;
